...
public class ModelPart extends AbstractFXContentPart<Group> {
	...
	protected void doRefreshVisual(Group visual) {
		...
		text.setFont(Font.font("Monospace", FontWeight.NORMAL, 50));
		...
		Rectangle bounds = new Rectangle(model.getPosition(),
				new Dimension(textBounds.getWidth()*1.5, textBounds.getHeight() * 1.5));

		// the rounded rectangle
		{
			RoundedRectangle roundRect = new RoundedRectangle(bounds, 10, 10);
			FXGeometryNode<RoundedRectangle> GeometryNode = new FXGeometryNode<>(roundRect);
			GeometryNode.setFill(model.getColor());
			GeometryNode.setStroke(Color.BLACK);
			GeometryNode.setStrokeWidth(1);
